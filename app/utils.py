...
def get_all_trials():
    try:
        file_path = 'trials_int.json'
        if not os.path.exists(file_path):
            logger.error(f"❌ trials_int.json file does not exist at {file_path}")
            return []

        with open(file_path, 'r') as f:
            trials = json.load(f)
            if not trials:
                logger.error(f"❌ trials_int.json file is empty.")
                return []

        logger.info(f"✅ Loaded {len(trials)} trials from {file_path}")
        return trials
    except FileNotFoundError:
        logger.error("❌ trials_int.json not found.")
        return []
    except json.JSONDecodeError:
        logger.error("❌ Invalid JSON format in trials_int.json.")
        return []
    except Exception as e:
        logger.error(f"❌ Unexpected error loading trials: {str(e)}")
        return []
