...
REGOLE FERREE:
- Restituisci UNO e SOLO UNO oggetto JSON valido.
- Nessun testo prima o dopo. Nessun markdown, nessun commento, nessun code fence.
- Se un valore non è ricavabile dal testo, scrivi esattamente "not mentioned" (anche per numerici).
- Ogni campo "*_source_text" deve contenere l’esatto frammento di testo da cui proviene il dato; se non presente, scrivi "not mentioned".
- Per le liste: usa sempre array JSON. Se nulla è menzionato → ["not mentioned"].
- Normalizza i nomi in inglese (es. “carboplatino” → “carboplatin”; “linfonodali” → “lymph nodes”).
...
{{
  ...
  "histology": "squamous" | "adenocarcinoma" | "small_cell" | "other" | "not mentioned",
  ...
  "metastases_sites": ["brain" | "liver" | "bone" | "lymph nodes" | "adrenal" | "pleural" | "pericardial"] | ["not mentioned"],
  "metastases_sites_source_text": "string",
...
- Estrarre SOLO trattamenti oncologici in "prior_systemic_therapies".
- Estrarre comorbidità SOLO dalla sezione "COMORBIDITÀ".
- Estrarre terapie concomitanti SOLO dalla sezione "terapie domiciliari".
- Per metastasi, preferire i reperti della TC/PET più recente.
- Qualsiasi informazione assente → "not mentioned".
...
def build_previous_treatments(previous_treatments: dict) -> List[str]:
    """
...
- For EACH EXCLUSION violated: −50 points.
...
IMPORTANT
-DEVI CONTROLLARE BENE SE IL PZ HA UNA SIGNIFICANT CARDIOVASCULAR DISEASES, CE LA HA SOLO SE è ESPRESSAMENTE SCRITTO
- RICORDATI CHE L'IPERTENSIONE ARTERIOSA NON è UNA MALATTIA CARDIACA!
- If any single exclusion is clearly violated, you may reduce to ≤50 immediately and set "Not Eligible".
- Prefer high precision on molecular prerequisites (e.g., KRAS G12C required).
...
def process_patient_document(text_or_file: Union[str, bytes], is_file: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        clean_features["previous_treatments"] = build_previous_treatments(pt) if pt else []
