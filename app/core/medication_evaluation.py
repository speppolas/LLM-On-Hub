...
def _write_outputs(per_feature: Dict[str, Dict], overall: Dict[str, float], out_dir: str, diagnostics: Optional[Dict] = None):
    ...
    payload = {
        ...
        "precision_weighted": _r2(overall.get("precision_weighted", 0)),
        "recall_weighted":    _r2(overall.get("recall_weighted", 0)),
        "f1_weighted":        _r2(overall.get("f1_weighted", 0)),
        "accuracy_weighted":  _r2(overall.get("accuracy_weighted", 0)),
    ...
    try:
        ...
        # 🔹 Escludi queste due feature solo dai grafici
        df_plot = df_plot[~df_plot["feature"].isin(["collateral", "comorbidities"])]

        if not df_plot.empty and plt is not None:
            # F1
            if {"feature","f1-score"} <= set(df_plot.columns):
                plt.figure(figsize=(8,4))
                plt.bar(df_plot["feature"].astype(str),
                        pd.to_numeric(df_plot["f1-score"], errors="coerce").fillna(0.0))
                plt.xticks(rotation=45, ha="right")
                plt.ylabel("F1 (macro)")
                plt.title("F1 per feature")
                plt.tight_layout()
            ...
            if "accuracy" in df_plot.columns:
                plt.figure(figsize=(8,4))
                plt.bar(df_plot["feature"].astype(str),
                        pd.to_numeric(df_plot["accuracy"], errors="coerce").fillna(0.0))
                plt.xticks(rotation=45, ha="right")
                plt.ylabel("Accuracy (macro)")
                plt.title("Accuracy per feature")
                plt.tight_layout()
                ...
                plt.close()
            # Precision
            if "precision" in df_plot.columns:
                plt.figure(figsize=(8,4))
                plt.bar(df_plot["feature"].astype(str),
                        pd.to_numeric(df_plot["precision"], errors="coerce").fillna(0.0))
                plt.xticks(rotation=45, ha="right")
                plt.ylabel("Precision (macro)")
                plt.title("Precision per feature")
                plt.tight_layout()
                ...
                plt.close()
            # Recall
            if "recall" in df_plot.columns:
                plt.figure(figsize=(8,4))
                plt.bar(df_plot["feature"].astype(str),
                        pd.to_numeric(df_plot["recall"], errors="coerce").fillna(0.0))
                plt.xticks(rotation=45, ha="right")
                plt.ylabel("Recall (macro)")
                plt.title("Recall per feature")
                plt.tight_layout()
    ...
    # ---- Heatmap filtrata ----
    try:
        df_h = pd.read_csv(os.path.join(out_dir, "med_metrics.csv"))
        df_h = df_h[~df_h["feature"].isin(["collateral", "comorbidity"])]
        if not df_h.empty:
            metrics = ["accuracy","precision","recall","f1-score"]
            df_m = df_h.set_index("feature")[metrics].astype(float)
            plt.figure(figsize=(8, len(df_m)*0.4+2))
            sns.heatmap(df_m, annot=True, fmt=".2f", cmap="YlGnBu", cbar=True)
            plt.title("Performance per feature")
            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, "heatmap_metrics.png"))
            plt.close()
    except Exception as e:
        print("WARN: heatmap non creata:", e)
    ...
    _plot_global_radar(
        overall,
        os.path.join(out_dir, "global_radar.png")
    )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "w") as zf:
            for fn in ("med_metrics.csv", "med_metrics_weighted.csv",
                       "global_metrics.json", "med_f1.png", "med_accuracy.png", "med_precision.png", "med_recall.png",
                       "exact_match.csv", "heatmap_metrics.png", "global_radar.png", "exact_match.json"):
                p = os.path.join(out_dir, fn)
                if os.path.exists(p):
                    zf.write(p, arcname=fn)
            for fn in os.listdir(out_dir):
